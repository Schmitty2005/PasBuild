...
var
  Units, FilteredUnits: TStringList;
...
begin
  ...
  try
    // Apply bootstrapExclude filter
    FilteredUnits := TStringList.Create;
    try
      for I := 0 to Units.Count - 1 do
      begin
        CurrentUnitName := Units[I];
        if AConfig.BuildConfig.BootstrapExclude.IndexOf(CurrentUnitName) < 0 then
          FilteredUnits.Add(CurrentUnitName);
      end;

      if FilteredUnits.Count = 0 then
      begin
        TUtils.LogWarning('No units found for bootstrap program');
        Exit;
      end;

      TUtils.LogInfo('Discovered ' + IntToStr(FilteredUnits.Count) + ' units');
      ...
      for I := 0 to FilteredUnits.Count - 1 do
      begin
        CurrentUnitName := FilteredUnits[I];
        if I < FilteredUnits.Count - 1 then
