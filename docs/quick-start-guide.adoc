...
=== Profiles in Multi-Module Projects

In <<_multi_module_projects,multi-module projects>>, profiles defined in the aggregator (parent) `project.xml` are automatically inherited by all child modules. This allows you to define shared profiles once at the top level rather than duplicating them in every module.
...
Both `framework` and `app` modules will have the `unix` profile available, without needing to redeclare it. Running `pasbuild compile -p unix` will apply the `-dX11` define to all child modules.
...
1. CLI flag: `--compiler <path>`
2. Environment variable: `PASBUILD_COMPILER`
...
pasbuild --version --compiler /opt/fpc-3.3.1/bin/fpc
----

**Using the PASBUILD_COMPILER environment variable:**
...
* `${build.time}` - Current time in `hh:mm:ss` format
...
**Example with build metadata** (`src/main/resources/build-info.inc`):

[source]
----
'Built ${build.date} at ${build.time}'
----

After processing, `target/build-info.inc` will contain something like:

[source]
----
'Built 2026-05-01 at 14:30:45'
----

Which can then be used in Pascal source as:

[source,pascal]
----
const
  BUILD_INFO = {$I build-info.inc};
----

**With filtering enabled** and `<name>MyApp</name>`, `<version>1.0.0</version>` in project.xml, a file containing all tokens:
...
* Version and build-date include files for Pascal `{$I}` directives
...
. Checks if `src/main/resources` exists (skips if not found)
...
. Checks if `src/test/resources` exists (skips if not found)
. Recursively copies all files preserving directory structure
. If filtering is enabled, substitutes project variables
. Reports files copied/filtered
...
. Compiles main code (via `compile` goal)
. Scans `src/test/pascal/` for test files
. Compiles test runner executable to `target/TestRunner`
. Links to already-compiled main units (Maven-style)
. Outputs test units to `target/test-units` (separate from main units)
...
. Detects the FPC target triplet (e.g., `x86_64-linux-3.2.2`)
. Creates the repository directory structure
. Copies all `.ppu` and `.o` files from `target/units/` to the repository
. Generates `metadata.xml` with artifact identity and dependency information
